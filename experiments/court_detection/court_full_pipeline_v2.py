...
def fit_line_from_segments(segs, w, h):
    """
    세그먼트 여러 개 → cv2.fitLine 으로 하나의 무한 직선 → 화면 경계 p1,p2, abc 형태로 반환
    """
    pts = []
    for s in segs:
        (x1,y1),(x2,y2) = _seg_get_p1p2(s)
    ...
    vx, vy, x0, y0 = cv2.fitLine(pts, cv2.DIST_L2, 0, 1e-2, 1e-2)
    vx, vy, x0, y0 = float(vx), float(vy), float(x0), float(y0)

    # a x + b y + c = 0 로 변환 (정규화)
    # 방향벡터 (vx,vy)에 수직인 법선은 (a,b) = (vy, -vx)
    a = vy
    b = -vx
    norm = math.hypot(a, b)
    if norm < 1e-9:
    ...
    # 화면 경계와 교차하는 두 점(p1,p2) 구하기
    def line_x_at_y(a,b,c,y):
        if abs(a) < 1e-9:
            return None
        return float(-(b*y + c)/a)

    def line_y_at_x(a,b,c,x):
        if abs(b) < 1e-9:
            return None
    ...
    candidates = []
    for y in [0, h-1]:
        x = line_x_at_y(a,b,c,y)
        if x is not None and -w <= x <= 2*w:
            candidates.append((int(round(x)), int(round(y))))
    for x in [0, w-1]:
        y = line_y_at_x(a,b,c,x)
        if y is not None and -h <= y <= 2*h:
            candidates.append((int(round(x)), int(round(y))))
    ...
    return {
        ...
        "p2": p2,
        "abc": (a,b,c),
        "score": 1.0,  # 필요하면 나중에 덮어쓰기
    }

def fit_sidelines_two_sides(segs_len, w, h, center_ratio=0.5, margin_ratio=0.1, min_cnt=4):
    """
    길이 방향 세그먼트(segs_len)를 왼/오로 나눠서 각각 하나씩 사이드라인 피팅.
    - center_ratio: x 기준으로 좌/우를 나누는 기준 (보통 0.5 = 화면 중앙)
    - margin_ratio: 양 끝쪽만 사용하고 싶을 때 (0.1 이면 양 끝 10%~40%, 60%~90% 구간 정도)
    """
    if len(segs_len) == 0:
        return []
    ...
    # 중앙 기준으로 단순 분리 (필요하면 k-means로 바꿀 수 있음)
    x_center = w * float(center_ratio)
    left_segs  = [s for s,mx in zip(segs_len, xs) if mx < x_center]
    right_segs = [s for s,mx in zip(segs_len, xs) if mx >= x_center]

    # 중앙부 라인(센터/싱글)을 빼고 싶으면 margin_ratio 활용
    xL_max = w * (0.5 - margin_ratio)
    xR_min = w * (0.5 + margin_ratio)
    left_segs  = [s for s,mx in zip(left_segs,  [0.5*(s[0]+s[2]) for s in left_segs])  if mx <= xL_max]
    right_segs = [s for s,mx in zip(right_segs, [0.5*(s[0]+s[2]) for s in right_segs]) if mx >= xR_min]

    lines = []

    if len(left_segs) >= min_cnt:
        L = fit_line_from_segments(left_segs, w, h)
        if L is not None:
            L["which_side"] = "L"
            lines.append(L)

    if len(right_segs) >= min_cnt:
        R = fit_line_from_segments(right_segs, w, h)
        if R is not None:
            R["which_side"] = "R"
            lines.append(R)
...
def make_court_roi(mask_u8: np.ndarray, top_cut_px: int = 30, dilate_k: int = 9) -> np.ndarray:
    ...
    # 심판석 배제 시도
    num, labels, stats, cents = cv2.connectedComponentsWithStats(roi, connectivity=8)
    ...
    for i in range(1, num):
        ...
        if area == 0:
            continue
        aspect = h_cc / float(max(1, w_cc))

        # 조건 예시 (필요하면 튜닝):
        #   - 세로로 꽤 길고(aspect > 3)
        #   - 폭이 전체 폭 대비 작고(w_cc < 8% of width)
        #   - 화면 상단 쪽까지 올라가 있음(y < 55% of height)
        if aspect > 3.0 and w_cc < 0.08 * w_img and y < 0.55 * h_img:
            roi[labels == i] = 0
...
def robust_mean_angle_mod180_from_segs(segs):
    """
    segs: [ [x1,y1,x2,y2], ... ] 또는 dict도 가능하면 segment_angle_signed_deg/_seg_get_p1p2 쓰는 쪽으로 확장 가능
    여기서는 main에서 segs가 [x1,y1,x2,y2]로 유지된다고 가정.
    반환: [0,180) 범위의 대표 각도 (deg)
    """
    if not segs:
        return 0.0
    thetas = []
    ...
    for (x1,y1,x2,y2) in segs:
        ...
        L = math.hypot(dx, dy)
        thetas.append(th)
        weights.append(max(1.0, L))

    # 가중 중앙값(랩어라운드 이슈 적음)
    order = sorted(range(len(thetas)), key=lambda i: thetas[i])
    thetas_s = [thetas[i] for i in order]
    weights_s = [weights[i] for i in order]
    tot = sum(weights_s)
    acc = 0.0
    med = thetas_s[len(thetas_s)//2]
    for th, w in zip(thetas_s, weights_s):
        acc += w
        if acc >= 0.5 * tot:
            med = th
            break
    ...
    # mod 180로 변환
    return float(med % 180.0)


def estimate_vanishing_point_from_lines(lines):
    """
    lines: [{"abc": (a,b,c), ...}, ...] where a,b normalized in fit_line_tls()
    최소제곱으로 한 점(vp)이 모든 라인 위에 가깝게 놓이도록 (a x + b y + c = 0) 풀기.
    homogeneous v = (x,y,1).  A v ≈ 0 의 SVD 해.
    """
    if len(lines) < 2:
        return None
...
def filter_lines_by_vp(lines, vp, max_dist_px=6.0):
    """
    vp가 각 라인 위에 얼마나 잘 놓이는지로 필터.
    fit_line_tls가 (a,b) 정규화 해주니 |a x + b y + c| 가 픽셀 거리로 해석 가능.
    """
    if vp is None:
        return lines
    x,y = vp
...
def main():
    ...
    parser.add_argument("--min_segs_horiz", type=int, default=8)
    parser.add_argument("--min_segs_diag",  type=int, default=3)
    parser.add_argument("--vp_max_dist", type=float, default=6.0, help="vanishing point consistency threshold (px)")

    parser.add_argument("--dt_min", type=float, default=0.5)
    ...
    # 2.6) Remove thick blobs -> keep only line-like band for Hough
    dist = cv2.distanceTransform((thick_for_hough > 0).astype(np.uint8), cv2.DIST_L2, 3)

    # dt_min ~ dt_max 사이만 선으로 사용 (가운데 뼈대 쪽을 살짝 두께 있게 남김)
    dt_min = getattr(args, "dt_min", 0.5)
    dt_max = getattr(args, "dt_max", 3.0)

    band = (dist >= dt_min) & (dist <= dt_max)
    thin_for_hough = (band.astype(np.uint8)) * 255

    # 너무 공격적인 open 대신, 살짝 closing 으로 깨진 부분만 이어주기
    k3 = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
    thin_for_hough = cv2.morphologyEx(thin_for_hough, cv2.MORPH_CLOSE, k3, iterations=1)

    save_png(out_dir, "03_2_thin_for_hough_dt", thin_for_hough)

    # Hough는 "엣지"에 주는 게 더 안정적
    edges_from_thin = cv2.Canny(thin_for_hough, 50, 150)
    edges_from_mask = cv2.Canny(mask_focus, 50, 150)

    save_png(out_dir, "03_3_edges_from_thin", edges_from_thin)
    save_png(out_dir, "03_3_edges_from_mask", edges_from_mask)
    ...
    # ----------------------------
    # 06) 3-way 클러스터별 세그먼트 분리
    # ----------------------------
    segsH = [s for s, lb in zip(segs, seg_labels) if lb == 0]  # H (가로)
    segsP = [s for s, lb in zip(segs, seg_labels) if lb == 1]  # +D
    segsN = [s for s, lb in zip(segs, seg_labels) if lb == 2]  # -D

    h, w = bgr.shape[:2]

    # --- 대표 각도(클러스터별) 계산 ---
    angH = robust_mean_angle_mod180_from_segs(segsH)
    angP = robust_mean_angle_mod180_from_segs(segsP)
    angN = robust_mean_angle_mod180_from_segs(segsN)

    save_json(out_dir, "06_cluster_angles_3way", {
        "angle_deg_mod180": {"H": angH, "+D": angP, "-D": angN},
        "counts": {"H": len(segsH), "+D": len(segsP), "-D": len(segsN)},
        "horiz_thr": float(args.horiz_thr),
    })

    # ----------------------------
    # 07) 클러스터별 rho-peak 기반 라인 후보
    # ----------------------------
    linesH, rhosH, peaksH = extract_parallel_lines_from_cluster(
        segsH, angH, w, h,
    ...
    linesP, rhosP, peaksP = extract_parallel_lines_from_cluster(
        segsP, angP, w, h,
        peak_bins=args.peak_bins, rho_tol=args.rho_tol,
        max_lines=args.max_lines_each,
        peak_min_prom=args.peak_min_prom, peak_min_dist=args.peak_min_dist
    )
    linesN, rhosN, peaksN = extract_parallel_lines_from_cluster(
        segsN, angN, w, h,
        peak_bins=args.peak_bins, rho_tol=args.rho_tol,
        max_lines=args.max_lines_each,
        peak_min_prom=args.peak_min_prom, peak_min_dist=args.peak_min_dist
    ...
    save_png(out_dir, "06_rho_hist_H",
             rho_hist_image(rhosH, bins=args.peak_bins, peaks=peaksH, title="rho_hist_H"))
    save_png(out_dir, "07_rho_hist_posD",
             rho_hist_image(rhosP, bins=args.peak_bins, peaks=peaksP, title="rho_hist_posD"))
    save_png(out_dir, "07b_rho_hist_negD",
             rho_hist_image(rhosN, bins=args.peak_bins, peaks=peaksN, title="rho_hist_negD"))

    # ----------------------------
    # 08) ROI 필터 및 길이방향 라인 보강 (좌/우 사이드라인 강제 피팅 포함)
    # ----------------------------

    # (1) ROI 필터 (클러스터별)
    linesH = filter_fitted_lines_by_roi(linesH, court_roi, min_score=0.25, thickness=9)
    linesP = filter_fitted_lines_by_roi(linesP, court_roi, min_score=0.25, thickness=9)
    linesN = filter_fitted_lines_by_roi(linesN, court_roi, min_score=0.25, thickness=9)

    # (2) 길이방향 세그먼트/라인 모으기
    segs_len = segsP + segsN

    # rho-peak 기반 길이방향 라인 (센터/싱글 포함)
    ang_len = robust_mean_angle_mod180_from_segs(segs_len)
    lines_len_rho, rhos_len, peaks_len = extract_parallel_lines_from_cluster(
        segs_len, ang_len, w, h,
        peak_bins=args.peak_bins, rho_tol=args.rho_tol,
        max_lines=args.max_lines_each,
        peak_min_prom=args.peak_min_prom, peak_min_dist=args.peak_min_dist
    )
    save_png(out_dir, "07c_rho_hist_lengthwise",
             rho_hist_image(rhos_len, bins=args.peak_bins, peaks=peaks_len, title="rho_hist_lengthwise"))

    # 좌/우 사이드라인은 반드시 한 개씩 나오도록 세그먼트에서 직접 피팅
    sidelines_lr = fit_sidelines_two_sides(
        segs_len, w, h,
        center_ratio=0.5,
        margin_ratio=0.12,
        min_cnt=args.min_segs_diag
    )

    # ROI 필터 다시 적용
    lines_len_rho = filter_fitted_lines_by_roi(lines_len_rho, court_roi, min_score=0.25, thickness=9)
    sidelines_lr   = filter_fitted_lines_by_roi(sidelines_lr,   court_roi, min_score=0.25, thickness=9)

    # 전체 길이방향 후보 = rho 기반 + 좌/우 보강 라인
    lengthwise_all = lines_len_rho + sidelines_lr

    # (선택) 소실점 일관성으로 한 번 더 정제
    vp = estimate_vanishing_point_from_lines(lengthwise_all)
    lengthwise_all = filter_lines_by_vp(lengthwise_all, vp, max_dist_px=args.vp_max_dist)

    save_json(out_dir, "08_vp_debug", {"vp_xy": vp, "n_lengthwise_kept": len(lengthwise_all)})
    vp_vis = bgr.copy()
    if vp is not None:
        vx, vy = int(round(vp[0])), int(round(vp[1]))
        cv2.circle(vp_vis, (vx, vy), 8, (0,255,255), -1, cv2.LINE_AA)
        put_label(vp_vis, f"VP ({vx},{vy})", max(10, vx+10), max(20, vy+10), (0,255,255))
    save_png(out_dir, "08c_vanishing_point", vp_vis)

    # (3) 디버그: 클러스터별 + 길이방향 라인 오버레이
    dbg_lines = bgr.copy()
    for L in linesH:
        cv2.line(dbg_lines, L["p1"], L["p2"], (0,255,0), 2, cv2.LINE_AA)   # H = green
    for L in linesP:
        cv2.line(dbg_lines, L["p1"], L["p2"], (0,0,255), 2, cv2.LINE_AA)   # +D = red
    for L in linesN:
        cv2.line(dbg_lines, L["p1"], L["p2"], (255,0,0), 2, cv2.LINE_AA)   # -D = blue
    save_png(out_dir, "08d_parallel_lines_3way_roi", dbg_lines)

    dbg_sidelines = bgr.copy()
    for L in sidelines_lr:
        col = (255,0,255) if L.get("which_side","L") == "L" else (0,255,255)
        cv2.line(dbg_sidelines, L["p1"], L["p2"], col, 3, cv2.LINE_AA)
    save_png(out_dir, "08e_sidelines_lr_from_segments", dbg_sidelines)

    # ----------------------------
    # 09~11) 라벨링 및 최종 오버레이
    # ----------------------------
    labeled_len = label_lengthwise_lines(lengthwise_all, w, h)
    labeled_crs = label_crosswise_lines(linesH, w, h)
...
python court_full_pipeline_v2.py --input fullcourt_wide.jpg --out_root results_full_v2 \
  --hough_thresh 35 --min_len 35 --max_gap 50 \
  --horiz_thr 18 \
  --min_segs_horiz 10 --min_segs_diag 3 \
  --vp_max_dist 7
"""