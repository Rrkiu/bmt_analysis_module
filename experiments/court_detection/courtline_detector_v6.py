...
def _get_best_sideline_x_bounds(lines: List[DetectedLine], calibration: CourtCalibration) -> Tuple[int, int]:
    """수평 라인을 '어느 사이드라인까지' 그릴지 결정.

    우선순위:
    1) 복식 사이드라인(left/right_doubles)이 있으면 그 x_center 사용
    2) 단식 사이드라인(left/right_singles)이 있으면 그 x_center 사용
    3) 둘 다 없으면 코트 규격(0m~5.18m)을 픽셀로 변환

    참고:
    - 이 함수는 '수평 라인(서비스/베이스라인 등)'을 full line으로 만들 때,
      좌/우 끝을 어디까지 그릴지 결정하기 위해 사용한다.
    """
    left = next((l for l in lines if l.category == 'sideline_left_doubles'), None)
    right = next((l for l in lines if l.category == 'sideline_right_doubles'), None)
    if left is None:
        left = next((l for l in lines if l.category == 'sideline_left_singles'), None)
    if right is None:
        right = next((l for l in lines if l.category == 'sideline_right_singles'), None)

    if left is not None and right is not None:
        x_left = int(round(left.x_center))
        x_right = int(round(right.x_center))
    else:
        # 규격 기반 (복식 전체 폭)
        x_left = calibration.meters_to_pixels(COURT_SPEC['line_positions']['sideline_left_doubles'], 0)[0]
        x_right = calibration.meters_to_pixels(COURT_SPEC['line_positions']['sideline_right_doubles'], 0)[0]

    # 안전 클램프
    x_left = max(0, min(calibration.warped_width - 1, x_left))
    x_right = max(0, min(calibration.warped_width - 1, x_right))
    if x_left > x_right:
...
def extend_horizontal_lines_full(lines: List[DetectedLine],
                                calibration: CourtCalibration,
                                categories: Optional[List[str]] = None,
                                force: bool = True) -> List[DetectedLine]:
    """숏/롱 서비스, 베이스라인(및 필요 시 net) 등을 '규격 기반 full line'으로 생성.

    동작 방식:
    - y 위치는 COURT_SPEC['line_positions'][category]를 사용해 규격 기반으로 고정한다.
    - x 범위는 (가능하면) 검출된 사이드라인의 x_center를 사용해 좌/우 경계까지 연장한다.
      (사이드라인이 없으면 코트 규격 0m~5.18m를 사용)

    force 옵션:
    - True: 해당 라인이 허프에서 검출/분류되지 않았더라도, 지정한 categories는 항상 full line으로 생성한다.
    - False: 해당 category가 실제로 검출(분류)된 경우에만 full line으로 생성한다.
    """
    if categories is None:
        categories = ['short_service', 'long_service', 'baseline']

    if not force:
        present = {c: any(l.category == c for l in lines) for c in categories}
        if not any(present.values()):
            return lines
    else:
        present = {c: True for c in categories}

    x_left, x_right = _get_best_sideline_x_bounds(lines, calibration)

    # 기존 라인 중 대상 카테고리는 제거
    result: List[DetectedLine] = [l for l in lines if l.category not in categories]

    # 대상 카테고리는 규격 기반으로 full line 생성
    for cat in categories:
        if not present.get(cat, False):
            continue

        y_m = COURT_SPEC['line_positions'][cat]
        y_px = calibration.meters_to_pixels(0, y_m)[1]
        y_px = max(0, min(calibration.warped_height - 1, int(round(y_px))))

        new_line = DetectedLine(
            p1=(x_left, y_px),
            p2=(x_right, y_px),
            angle=0.0,
            length=float(abs(x_right - x_left)),
...
def process_test_case(test_case, base_output_dir="."):
    ...
    # 출력 디렉토리 생성
    timestamp = datetime.now().strftime("%y%m%d_%H%M%S")
    output_dir = os.path.join(base_output_dir, f"court_lines_v6_{name}_{timestamp}")
    os.makedirs(output_dir, exist_ok=True)
    ...
    # Step 3.5: Hough Line 검출 결과 시각화
    print("\n[Step 3.5] Visualize Hough Lines")
    
    # 03a: Bird's Eye View 이미지에 허프 라인 표시
    hough_vis_image = image_warped.copy()
    for line in detected_lines:
        cv2.line(hough_vis_image, line.p1, line.p2, (0, 255, 0), 2)
    cv2.imwrite(os.path.join(output_dir, "03a_hough_on_birdseye.jpg"), hough_vis_image)
    print(f"  Saved: 03a_hough_on_birdseye.jpg ({len(detected_lines)} lines on warped image)")
    
    # 03b: Canny 엣지(Bird's Eye View)에 허프 라인 표시
    hough_vis_edges = cv2.cvtColor(edges_warped_gray, cv2.COLOR_GRAY2BGR)
    for line in detected_lines:
        cv2.line(hough_vis_edges, line.p1, line.p2, (0, 255, 0), 2)
    cv2.imwrite(os.path.join(output_dir, "03b_hough_on_edges.jpg"), hough_vis_edges)
    print(f"  Saved: 03b_hough_on_edges.jpg ({len(detected_lines)} lines on edge image)")
    ...
    # Step 5.7: 수평 라인(숏/롱 서비스, 베이스라인) 규격 기반 full line 생성
    print("\n[Step 5.7] Extend Horizontal Lines (Spec Based Full Lines)")
    main_lines = extend_horizontal_lines_full(main_lines, calibration, categories=['short_service', 'long_service', 'baseline'], force=True)
...
# python courtline_detector_v5_cv_w_baseline.py  /mnt/b/cd_p/bmt_demo/stage1_courtdetection/niceangle_wide_halfcourt.json

"""
python courtline_detector_v6.py \
  /mnt/b/cd_p/bmt_demo/experiments/stage1_courtdetection/niceangle_wide_halfcourt.json \
  -o results
"""
