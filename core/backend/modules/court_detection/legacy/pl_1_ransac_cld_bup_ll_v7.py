...
def save_image(out_dir: Path, name: str, img: np.ndarray):
    """Save image to output directory. Skip if out_dir is None (API mode)."""
    if out_dir is None:
        return  # Skip saving in API mode
...
def ransac_final_line_fit(points: np.ndarray, dist_th: float = 3.0,
                          max_iter: int = 500, min_inliers: int = 20):
    """
    Final RANSAC line fitting to remove remaining outliers.
    
    This is applied AFTER all other filtering steps to ensure the final
    line fit is not skewed by any remaining noise (e.g., advertisement 
    board pixels that passed earlier filters).
    
    Args:
        points: Nx2 array of (x, y) coordinates
        dist_th: Inlier distance threshold in pixels
        max_iter: Maximum RANSAC iterations
        min_inliers: Minimum number of inliers to accept
    
    Returns:
        (p0, d, n, inlier_pts) or None if failed
    """
    if len(points) < min_inliers:
        # Not enough points, just fit directly
        fit = fit_line_from_points(points)
        if fit is None:
            return None
        p0, d, n = fit
        return p0, d, n, points
    ...
    if best_model is None or best_count < min(min_inliers, n_pts // 2):
...
def bottom_up_sideline_extraction(points: np.ndarray, H: int, region_type: str,
    ...
    4. Apply local linearity filter to remove noise clusters
    5. **NEW: Final RANSAC to remove remaining outliers (e.g., ad board noise)**
    6. Final line fit on RANSAC inliers only
    ...
    - **Final RANSAC removes any remaining outliers that skew the line fit**
    ...
    Args:
        ...
        k_neighbors: Number of neighbors for local linearity check
        linearity_th: Max residual threshold for linearity filter
        final_ransac_dist_th: Distance threshold for final RANSAC (pixels)
        final_ransac_iter: Number of RANSAC iterations for final fit
    ...
    # Step 4: Final line fit with RANSAC (IMPROVED - removes outliers like ad noise)
    print(f"    Applying final RANSAC to remove outliers...")
    
    ransac_result = ransac_final_line_fit(
        extended_pts,
        dist_th=final_ransac_dist_th,
        max_iter=final_ransac_iter,
        min_inliers=max(20, len(extended_pts) // 5)
    )
    
    if ransac_result is None:
        # Fallback to simple fit
        print(f"    [WARNING] Final RANSAC failed, using simple fit")
        final_line = fit_line_from_points(extended_pts)
        if final_line is None:
            raise RuntimeError(f"Failed to fit final line for {region_type} sideline.")
        p0, d, n = final_line
        final_pts = extended_pts
    else:
        p0, d, n, final_pts = ransac_result
        removed_count = len(extended_pts) - len(final_pts)
        if removed_count > 0:
            print(f"    Final RANSAC: {len(extended_pts)} -> {len(final_pts)} points (removed {removed_count} outliers)")
        else:
            print(f"    Final RANSAC: {len(final_pts)} inliers (no outliers removed)")
    
    # Save final inliers visualization
    if out_dir is not None:
        vis_final = np.zeros((H, max(int(points[:, 0].max()) + 50, 1920)), dtype=np.uint8)
        for pt in final_pts.astype(np.int32):
            if 0 <= pt[1] < vis_final.shape[0] and 0 <= pt[0] < vis_final.shape[1]:
                vis_final[pt[1], pt[0]] = 255
        save_image(out_dir, f"{debug_prefix}_03_final_ransac_inliers", vis_final)
    
    print(f"    Final line fitted with {len(final_pts)} points")
    
    return p0, d, n, final_pts
...
def filter_isolated_clusters(points: np.ndarray, y_bin_size: int = 20,
                              min_cluster_ratio: float = 0.1,
                              max_gap_bins: int = 3):
    """
    Filter out isolated small clusters from detected points.
    
    Keep only the main continuous region of points, removing:
    - Small isolated clusters at top/bottom
    - Noise clusters separated by gaps from the main line
    
    Args:
        points: Nx2 array of detected points
        y_bin_size: Height of y-bins for analysis
        min_cluster_ratio: Minimum ratio of total points to keep a cluster
        max_gap_bins: Maximum allowed gap (in bins) within a cluster
    
    Returns:
        filtered_points: Points from the main continuous region
    """
    if len(points) < 20:
        return points
    ...
    # Create y-bins
    n_bins = int(np.ceil((y_max - y_min) / y_bin_size))
    if n_bins < 3:
        return points
    ...
    for i, pt in enumerate(points):
        bin_idx = int((pt[1] - y_min) / y_bin_size)
        bin_idx = min(bin_idx, n_bins - 1)
        bin_counts[bin_idx] += 1
        bin_indices[bin_idx].append(i)
    
    # Find connected regions (clusters)
    # A cluster is a sequence of bins with points, allowing small gaps
    clusters = []
    ...
    for i in range(n_bins):
        if bin_counts[i] > 0:
            ...
            gap_count += 1
            if gap_count > max_gap_bins and current_cluster_start is not None:
                # End current cluster
                cluster_end = i - gap_count
                clusters.append((current_cluster_start, cluster_end))
                current_cluster_start = None
    
    # Close last cluster
    if current_cluster_start is not None:
        clusters.append((current_cluster_start, n_bins - 1))
    ...
    for start, end in clusters:
        size = sum(bin_counts[start:end+1])
    ...
    # Also keep clusters that are large enough (> min_cluster_ratio of total)
    # AND connected to the largest cluster
    min_size = total_points * min_cluster_ratio
    
    # Collect indices from the main cluster
    main_start, main_end = largest_cluster
    keep_indices = []
    
    for bin_idx in range(main_start, main_end + 1):
        keep_indices.extend(bin_indices[bin_idx])
    
    if not keep_indices:
        return points
    
    filtered_points = points[np.array(keep_indices)]
    
    print(f"      [Cluster Filter] {len(points)} -> {len(filtered_points)} points "
          f"(removed {len(points) - len(filtered_points)} from isolated clusters)")
...
def compute_endpoints_line_equation(p0: np.ndarray, d: np.ndarray,
    ...
    **KEY IMPROVEMENT**: 
    ...
    - **NEW**: Filter isolated clusters before computing y-range to ignore noise
    
    Strategy:
    1. **Filter isolated clusters** (removes noise at top/bottom)
    2. Find the y-range of the MAIN cluster of detected points
    3. Extrapolate the fitted line to extend beyond detected points
    4. Use image boundaries (with margin) as the endpoint y-coordinates
    5. Compute x at those y-coordinates using line equation
    ...
    Args:
        ...
        filter_clusters: If True, filter isolated clusters before computing y-range
    ...
    # NEW: Filter isolated clusters to get main continuous region
    if filter_clusters:
        filtered_pts = filter_isolated_clusters(inlier_pts)
        if len(filtered_pts) < 5:
            filtered_pts = inlier_pts  # Fallback
    else:
        filtered_pts = inlier_pts
    
    # Get y-range of filtered points (main cluster only)
    pts_y_min = filtered_pts[:, 1].min()
    pts_y_max = filtered_pts[:, 1].max()
...
def estimate_4pts_from_mask(mask255: np.ndarray, base_vis: np.ndarray, 
    ...
    **BOTTOM-UP SIDELINE EXTRACTION APPROACH**
    ...
    L_p0, L_d, L_n, L_pts = bottom_up_sideline_extraction(
        ...
        final_ransac_dist_th=args.final_ransac_dist_th,
        final_ransac_iter=args.final_ransac_iter,
        out_dir=out_dir,
    ...
    # NEW: Endpoints overlay with detected points only (no infinite lines)
    vis_endpoints_points = to_bgr(base_vis.copy())
    # Draw detected points
    for pt in L_pts.astype(np.int32):
        if 0 <= pt[1] < H and 0 <= pt[0] < W:
            cv2.circle(vis_endpoints_points, (pt[0], pt[1]), 2, (255, 255, 0), -1)  # Cyan filled
    for pt in R_pts.astype(np.int32):
        if 0 <= pt[1] < H and 0 <= pt[0] < W:
            cv2.circle(vis_endpoints_points, (pt[0], pt[1]), 2, (0, 255, 255), -1)  # Yellow filled
    
    # Draw endpoints
    draw_point(vis_endpoints_points, TL, (0, 0, 255))     # Red
    draw_point(vis_endpoints_points, TR, (0, 255, 0))     # Green
    draw_point(vis_endpoints_points, BL, (0, 0, 255))     # Red
    draw_point(vis_endpoints_points, BR, (0, 255, 0))     # Green
    
    save_image(out_dir, "08a_endpoint_overlay", vis_endpoints_points)
    ...
    # Detailed text file (skip if out_dir is None)
    if out_dir is not None:
    ...
    # Compact JSON-like file (skip if out_dir is None)
    if out_dir is not None:
...
def main():
    ...
    # Final RANSAC Parameters (NEW - removes outliers before line fitting!)
    parser.add_argument("--final_ransac_dist_th", type=float, default=3.0,
                        help="Distance threshold for final RANSAC line fitting (pixels). "
                             "Smaller = stricter, removes more outliers.")
    parser.add_argument("--final_ransac_iter", type=int, default=500,
                        help="Number of RANSAC iterations for final line fitting.")
