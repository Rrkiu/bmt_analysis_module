    ...
    for line in lines:
        ...
        if angle <= angle_threshold or angle >= (180 - angle_threshold):
            ...
            # 좌측 사이드라인 (단식/복식 구분)
                ...
                line.category = 'sideline_left_doubles'   # 복식 외곽
            elif 0.05 <= x_ratio < 0.15:
                line.category = 'sideline_left_singles'   # 단식 내곽
            # 우측 사이드라인 (단식/복식 구분)
            elif x_ratio > 0.95:
                line.category = 'sideline_right_doubles'  # 복식 외곽
            elif 0.85 < x_ratio <= 0.95:
                line.category = 'sideline_right_singles'  # 단식 내곽
            else:
...
def merge_lines_by_category(lines, threshold=30):
    ...
    for cat, group in groups.items():
        ...
        if cat in horizontal_categories:
...
def extend_sidelines_to_boundaries(lines, warped_width, warped_height):
    ...
    for sideline in sidelines:
        ...
        if abs(dy) > 1e-10:
            ...
            print(f"    {sideline.category} extended: x_top={x_top:.1f}, x_bottom={x_bottom:.1f}")
...
def draw_lines(image, lines):
    ...
    colors = {
        ...
        'center': (255, 0, 0),              # Blue
        # 복식 사이드라인 - Magenta
        'sideline_left_doubles': (255, 0, 255),
        'sideline_right_doubles': (255, 0, 255),
        # 단식 사이드라인 - Light Pink
        'sideline_left_singles': (255, 150, 255),
...
def process_test_case(test_case, base_output_dir="."):
    ...
    # 출력 디렉토리 생성 (테스트 케이스별)
    timestamp = datetime.now().strftime("%y%m%d_%H%M%S")
    output_dir = os.path.join(base_output_dir, f"court_lines_v4_{name}_{timestamp}")
    os.makedirs(output_dir, exist_ok=True)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='배드민턴 코트 라인 검출 v4 - JSON 설정 파일 기반 (단식/복식 사이드라인 구분)'
    )
    parser.add_argument(
        'json_path',
        type=str,
